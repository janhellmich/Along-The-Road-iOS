...
class MapViewController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate, UIGestureRecognizerDelegate {
    ...
    @IBAction func handleSliderRelease(sender: UISlider) {
        ...
        let distance = mapHelpers.milesToMeters(Double(sender.value))
        for (idx, waypoint) in waypoints.enumerate() {
            if waypoint.distance >= distance {
                print("SET ACTIVE WP called from handleSilider")
...
//        rightArrow.imageEdgeInsets = UIEdgeInsetsMake(-10, -10, -10, -10)
        ...
        let distance = routeData.route!.distance as Double
        ...
        print("VIEW DID LOAD CALLED: destitem is set? \(destinationItem == nil)")
        
        let listViewButton:UIBarButtonItem = UIBarButtonItem(image: UIImage(named: "list"), style: UIBarButtonItemStyle.Plain, target: self, action: "showListView")
...
//        }
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        if self.destinationItem != nil {
            updateMarkers()
            activeRestaurantIdx = -1
            print("SET ACTIVE WP called from viewWillAppear")
...
//    }
    ...
    */
    ...
    func displayRoute() {
        ...
        print("SET ACTIVE WP called from displayRoute")
    ...
    @IBAction func showDetailView(sender: AnyObject) {
        if restaurantData.filteredRestaurants.count > activeRestaurantIdx {
            let currentVenue = restaurantData.filteredRestaurants[activeRestaurantIdx]
    ...
    func mapView(mapView: MKMapView, didSelectAnnotationView view: MKAnnotationView) {
        ...
        for (idx, restaurant) in restaurantData.filteredRestaurants.enumerate() {
    ...
    */
    func setNewRegion () {
        //Extract the coord
        let startCoord = startItem?.placemark.coordinate
        let destCoord = destinationItem?.placemark.coordinate
        //All elements to be displayed on the map need to be placed in this array
        let locations = [startCoord!, destCoord!]
        


        let region = dataProcessor.findRegion(locations)
    ...
    */
    func sendFourSquareRequest (waypoint: WaypointStructure) {
        ...
        let long = waypoint.coordinate.longitude
        
        let url = NSURL(string: "https://api.foursquare.com/v2/venues/explore?client_id=\(self.CLIENT_ID)&client_secret=\(self.CLIENT_SECRET)&v=20150902&ll=\(lat),\(long)&venuePhotos=1&section=\(routeData.searchSection)&limit=20&radius=\(routeData.searchRadius)")
        ...
        NSURLConnection.sendAsynchronousRequest(req, queue: NSOperationQueue.mainQueue()) {(response, data, error) in
            ...
            let dataObj : AnyObject? = parsedObject?.objectForKey("response")?.objectForKey("groups")?[0].objectForKey("items")!
            ...
            let newlyAdded = self.restaurantData.addRestaurants(dataObj, waypointDistance: waypoint.distance)
            var newlyFiltered = self.filter.filterRestaurantArray(newlyAdded)
            newlyFiltered.sortInPlace({$1.totalDistance > $0.totalDistance})
            
            // sort newlyFiltered by distance
            print("NUM OF WPS: \(self.waypoints.count), LAST WP DIST: \(self.mapHelpers.metersToMiles(self.waypoints[self.waypoints.count - 1].distance))")
            print("FourSquare request returned \(newlyFiltered.count) results for WP #\(self.queryIndex)\n")
            if self.restaurantData.filteredRestaurants.count > 0 {
                 print("LAST RESTAURANT DIST: \(self.mapHelpers.metersToMiles(self.restaurantData.filteredRestaurants[self.restaurantData.filteredRestaurants.count-1].totalDistance))")
            }
            
            
            for restaurant in newlyFiltered  {
                print("REST DIST: \(self.mapHelpers.metersToMiles(restaurant.totalDistance))")
    ...
    func queryNext () {
        if (queryIndex < waypoints.count) {
            let waypoint = waypoints[queryIndex]
    ...
    func determineActiveRestaurant () {
        ...
        print("determineActiveRestaurant called: WP_IDX: \(activeWaypointIdx) WP_DIST: \(mapHelpers.metersToMiles(waypoints[activeWaypointIdx].distance))")
        for (idx, restaurant) in self.restaurantData.filteredRestaurants.enumerate() {
            ...
            if restaurant.totalDistance >= waypoints[activeWaypointIdx].distance && restaurant.totalDistance - waypoints[activeWaypointIdx].distance <= Double(routeData.searchRadius) {
                print("   active restaurant found: REST_DIST: \(mapHelpers.metersToMiles(restaurant.totalDistance))")
        ...
        if (activeWaypointIdx + 1 < waypoints.count) {
            print("SET ACTIVE WP called from determine active after no matching results found")
            setActiveWaypoint(activeWaypointIdx + 1)
        } else {
            print("EDGE CASE IN DETERMINE_ACTIVE_REST")
            if restaurantData.filteredRestaurants.count > 0 {
                let lastRestaurantDistance = restaurantData.filteredRestaurants[restaurantData.filteredRestaurants.count-1].totalDistance
                for (idx, waypoint) in waypoints.enumerate() {
    ...
    func updateMarkers () {
        ...
        print("updateMarkers called: # of Filtered: \(restaurantData.filteredRestaurants.count)")
    }
    ...
    func setActiveWaypoint (idx: Int) {
        ...
        let activeWaypoint = waypoints[activeWaypointIdx]
    ...
    func centerMap (waypoint: WaypointStructure) {
        let region = MKCoordinateRegionMakeWithDistance(waypoint.coordinate, 100, routeData.mapWidth)
        map.setRegion(region, animated: true)
    }
    
    func addActiveMarker() {
        if annotations.count > activeRestaurantIdx {
            let marker = makeAnnotation(annotations[activeRestaurantIdx].coordinate, imageName: "active")
            activeMarker = marker
    ...
    func setActiveRestaurant (idx: Int) {
        print("setActiveRestaurant called: REST_IDX: \(idx)/\(restaurantData.filteredRestaurants.count), REST_DIST: \(mapHelpers.metersToMiles(restaurantData.filteredRestaurants[idx].totalDistance))")
        ...
        let activeRestaurant = restaurantData.filteredRestaurants[idx]
        ...
        // request the image
        let imgURL: NSURL = NSURL(string: activeRestaurant.imageUrl)!
        ...
        if (Array(activeRestaurant.openUntil.lowercaseString.characters)[0] == "c") {
    ...
    @IBAction func goToNext(sender: UIButton) {
        print("goToNext called: REST_IDX: \(activeRestaurantIdx)/\(restaurantData.filteredRestaurants.count), WP_IDX: \(activeWaypointIdx)/\(waypoints.count)")
        if activeRestaurantIdx != -1 {
            if (activeRestaurantIdx + 1 < restaurantData.filteredRestaurants.count) {
                activeRestaurantIdx++
                let activeRestaurant = restaurantData.filteredRestaurants[activeRestaurantIdx]
                
                if activeWaypointIdx + 1 < waypoints.count {
                    let nextWaypoint = waypoints[activeWaypointIdx + 1]
                    if (activeRestaurant.totalDistance > nextWaypoint.distance) {
                        activeRestaurantIdx = -1
                        print("setActiveWaypoint called from nextButton")
                        setActiveWaypoint(activeWaypointIdx + 1)
                    } else {
    ...
    @IBAction func goToPrevious(sender: UIButton) {
        print("goToPrevious called: REST_IDX: \(activeRestaurantIdx)/\(restaurantData.filteredRestaurants.count), WP_IDX: \(activeWaypointIdx)/\(waypoints.count)")
